    ...
    return (
        <div className="relative w-[90%] h-[35%] md:h-[50%] lg:w-full lg:h-[85%] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 grid-rows-[repeat(auto-fill,minmax(1fr))] gap-4 p-5">
            {countdataServices.map((ele, i) => (
                <div
                    key={i}
                    className="hover:bg-orange-600 hover:scale-[1.05] flex flex-col justify-start items-start bg-[#141414] w-full h-full text-white p-5 rounded-md shadow-md hover:shadow-lg transition-transform duration-300"
                >
                    <img
                        src={ele.imagenRuta}
                        alt={ele.titulo}
