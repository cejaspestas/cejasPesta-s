...
// Función para transformar las URLs de YouTube
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-6">
            {countdataVideo.length > 0 ? (
                countdataVideo.map((ele, i) => (
                    <div key={i} className="bg-[#141414] text-white p-6 rounded-md shadow-md hover:shadow-lg transition-shadow duration-300">
                        {ele.videoRuta ? (
                            <iframe
                                width="100%"
                                height="315"
                                src={transformYouTubeUrl(ele.videoRuta)}
                        ...
                        <div className="mt-4">
                            <h2 className="text-lg font-bold font-mono mb-2">{ele.titulo || "Sin título"}</h2>
                            <p className="text-gray-400 text-sm mb-4 font-mono">{ele.descripcion || "Sin descripción"}</p>
                        </div>
                    </div>
                ))
            ) : (
