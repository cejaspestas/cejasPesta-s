...
export const NavMobile = () => {
  ...
  const router = usePathname() // Instancia de useRouter

  useEffect(() => {
    setOpen(false);
  }, [router]); // Detectar cambios en la ruta
          ...
          <div className="flex flex-col gap-y-10 w-full h-screen items-center justify-start mt-10">
            {dataRoutes.map((route) => {
              ...
              const itemClass = isSorteos
                ? "text-white hover:text-orange-500 bg-orange-500 px-5 py-2 rounded-md font-serif"
                : "text-white font-mono text-xl hover:text-gray-300 py-2";
